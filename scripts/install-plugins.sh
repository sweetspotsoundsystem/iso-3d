#!/bin/bash
# Install Iso3D plugins to system plugin directories
...
echo "Installing Iso3D plugins from $BUILD_DIR..."
echo ""
...
if [[ -d "$AU_PLUGIN" ]]; then
    ...
    mkdir -p "$AU_DEST"
    rm -rf "$AU_DEST/Iso3D.component"
    cp -R "$AU_PLUGIN" "$AU_DEST/"
    echo "  ✓ Installed to $AU_DEST/Iso3D.component"

    # Sign the plugin (required for DAW compatibility on macOS)
    echo "  Signing AU plugin..."
    # Sign embedded frameworks first
    if [[ -d "$AU_DEST/Iso3D.component/Contents/Frameworks" ]]; then
        find "$AU_DEST/Iso3D.component/Contents/Frameworks" -type f \( -name "*.dylib" -o -perm +111 \) -exec codesign --force --sign "$SIGN_IDENTITY" {} \;
    fi
    # Sign the main bundle
    codesign --force --sign "$SIGN_IDENTITY" "$AU_DEST/Iso3D.component"
...
if [[ -d "$VST3_PLUGIN" ]]; then
    ...
    mkdir -p "$VST3_DEST"
    rm -rf "$VST3_DEST/Iso3D.vst3"
    cp -R "$VST3_PLUGIN" "$VST3_DEST/"
    echo "  ✓ Installed to $VST3_DEST/Iso3D.vst3"

    # Sign the plugin (required for DAW compatibility on macOS)
    echo "  Signing VST3 plugin..."
    # Sign embedded frameworks first
    if [[ -d "$VST3_DEST/Iso3D.vst3/Contents/Frameworks" ]]; then
        find "$VST3_DEST/Iso3D.vst3/Contents/Frameworks" -type f \( -name "*.dylib" -o -perm +111 \) -exec codesign --force --sign "$SIGN_IDENTITY" {} \;
    fi
    # Sign the main bundle
    codesign --force --sign "$SIGN_IDENTITY" "$VST3_DEST/Iso3D.vst3"
    if [[ "$SIGN_IDENTITY" == "-" ]]; then
